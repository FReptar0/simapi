...
public class PacientesController {
    ...
    public ResponseEntity<CustomResponse<Pacientes>> createPatient(@RequestBody Pacientes patient){
        ...
        Pacientes patientCreate = pacientesRepository.findById(patient.getIdPaciente()).orElse(null);
        try {
            if(patient.getNombre().isEmpty() || patient.getApellidos().isEmpty() || patient.getDescripcion().isEmpty()){
                response.setError(true);//error = true
                response.setStatusCode(400); 
                response.setMessage("Verifica que los campos estén llenos");
                response.setData(patient);
            } else if(patientCreate != null){
                response.setError(true);
                response.setStatusCode(400);
                response.setMessage("Paciente ya existe");
                response.setData(patientCreate);//error = true 
    ...
    public ResponseEntity <CustomResponse<List<Pacientes>>> getPacientes(){
        ...
        try{
            ...
            return new ResponseEntity<>(response, HttpStatus.INTERNAL_SERVER_ERROR);// HttpStatus.INTERNAL_SERVER_ERROR = 500
        }
    }
    ...
    @RequestBody Pacientes paciente){
        ...
        try{
            ...
            if(pacienteUpdate == null){
                ...
                pacienteUpdate.setIdPaciente(paciente.getIdPaciente());
                response.setError(false);
                response.setStatusCode(200);
